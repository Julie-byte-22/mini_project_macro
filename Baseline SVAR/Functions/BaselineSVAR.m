...
while control==0
    ...
    a = (candidateirf(1,4,restr,k) > 0).* (candidateirf(2,4,restr,k) > 0).* (candidateirf(4,4,restr,k) > 0);
    if (max(a)==0)
      %--- Swiching the sign of the shock.
      am = (candidateirf(1,4,restr,k) < 0).* (candidateirf(2,4,restr,k) < 0).* (candidateirf(4,4,restr,k) < 0);
