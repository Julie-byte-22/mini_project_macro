...
while control==0
    ...
    a = (candidateirf(1,1,restr,k) > 0).* (candidateirf(2,1,restr,k) < 0).* (candidateirf(3,1,restr,k) > 0);
    if (max(a)==0)
      %--- Swiching the sign of the shock.
      am = (candidateirf(1,1,restr,k) < 0).* (candidateirf(2,1,restr,k) > 0).* (candidateirf(3,1,restr,k) < 0);
    ...
    a = (candidateirf(1,4,restr,k) > 0).* (candidateirf(2,4,restr,k) > 0).* (candidateirf(3,4,restr,k) > 0).* (candidateirf(4,4,restr,k) > 0);
    if (max(a)==0)
      %--- Swiching the sign of the shock.
      am = (candidateirf(1,4,restr,k) < 0).* (candidateirf(2,4,restr,k) < 0).* (candidateirf(3,4,restr,k) < 0).* (candidateirf(4,4,restr,k) < 0);
    ...
    elseif (min(a)==0)
      continue;  %The restrictions are not satisfied.  Go to the beginning to redraw.
         ...
         %%%%%%%%%%%%% Responses to a Super Stars Shock %%%%%%%%%%%%%:
   
    a = (candidateirf(1,5,restr,k) > 0).* (candidateirf(2,5,restr,k) > 0).* (candidateirf(3,5,restr,k) < 0).* (candidateirf(4,5,restr,k) > 0).* ((candidateirf(2,5,restr,k)+candidateirf(3,5,restr,k)-candidateirf(1,5,restr,k)) < 0);
    if (max(a)==0)
      %--- Swiching the sign of the shock.
      am = (candidateirf(1,5,restr,k) < 0).* (candidateirf(2,5,restr,k) < 0).* (candidateirf(3,5,restr,k) > 0).* (candidateirf(4,5,restr,k) < 0).* ((candidateirf(2,5,restr,k)+candidateirf(3,5,restr,k)-candidateirf(1,5,restr,k)) > 0);
